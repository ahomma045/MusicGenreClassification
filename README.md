...
#### (1) Spotify Tabular Audio Feature Classification
The Spotify data was scraped from Spotify playlist "Top 10,000 Songs of All Time" by Acclaimed Music, using the Spotipy API.

#### (2) Audio Data Classification
...
### (1) Spotify Tabular Data Classification
In this notebook we seek to make predictive models that do not rely on time-frequency analysis. We will use Spotify's API in order to collect song data to fit a logistic regression and a dense neural network models to discern genre. This process will serve as a benchmark of success for our other neural network models trained on raw audio data.

#### **Data Collection, Preprocessing, and Exploratory Data Analysis (EDA)**
We will use the "Top 10,000 Songs of All Time" playlist from Aclaimed Music to create a data frame with features for our model. We chose this playlist because it provides a comprehensive dataset without knowledge of other individual playlists. Ideally, we would filter through multiple playlists and add new songs to our existing dataset that are not already present. However, we lack a list of playlists, and the Million Playlists Dataset would require entry to the Alcrowd competition, which is not the focus of our project. Instead, we want to create an alternative predictive model to compare with our CNN model.
...
- The model achieved an accuracy of 0.490. 
- The model is an improvement from the base case at 0.293, but it is still not ideal. Our goal is not to create a perfect logistic regression model, but to compare it with other models using tabular data.

(b) Dense Neural Network
- The DNN model achieved a higher accuracy of 0.5155. 
- We did not need to regularize the model because it was not prone to overfitting. While we could improve the score further by adding batch sizing, dropouts, early stoppage, and more layers, that's not our goal for this project.

![spotify-data](/images/spotify-loss.png)
![spotify-data](/images/spotify-accuracy.png)

#### **Evaluation**
Below are the final accuracy scores for the models.
...
#### **Conclusion**
Classifying music using tabular data is better than the base case, and these models can be further improved with iteration. However, using tabular data requires music preprocessing to find the features needed for fitting, which takes time and resources. Additionally, the genre of the music can be determined during this preprocessing phase. Our goal for this project is to provide a user-friendly model for classifying lesser-known artists. We will compare our CNN models to these tabular data models to see how they perform.
...
- The model achieved an accuracy of 0.578. 
![DNN](/images/dnn.png)

(b) Convolutional Neural Network (Without Regularization)
...
- The model achieved an accuracy of 0.691. 
![CNN-1](/images/cnn-1.png)

(c) Convolutional Neural Network (With Regularization)
- A Convolutional Neural Network (CNN) consists of a combination of convolution layers and dense layers.
- Convolution layers first begin to identify features within the data, similar to edge detection in image data.
- The the data is flattened out to run through dense layers to classify the data.
...
- The model achieved an accuracy of 0.801.
![CNN-2](/images/cnn-2.png)

(d) Convolutional Neural Network (With Regularization and Data Augmentation)
...
- The model achieved an accuracy of 0.838. 
